# Canonicalizes candidate skills and scores required-skill hits via the taxonomy.
...
class SkillMatcherService:
    # Matches candidate skills to required skills using taxonomy relations.
    # Loads taxonomy nodes used to canonicalize and relate skills.
    def __init__(self, taxonomy: SkillTaxonomyLoader) -> None:
        self.taxonomy = taxonomy
        if not self.taxonomy.nodes:
            self.taxonomy.load()

    # Score required-skill coverage and experience quality for one candidate.
    def match(
        self,
